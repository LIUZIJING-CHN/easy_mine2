    ...
    for augs in range(n_aug):
        ...
        for batch_idx, (data, target) in enumerate(loader):        
            with torch.no_grad():
                ...
                # print(data.shape)
                features = model.backbone.forward(data)
                # print('feature1')
                ####
                ...
                #         feature_item_list = []
                #         feature_item_avg = features[i].unsqueeze(0)
                #         feature_item_list.append(feature_item_avg)  ##加入原有的特征
                        
                #         top_v, top_ind = torch.topk(feature_item_avg.squeeze(), k=20, dim=-1, largest=True)
                #         top_ind = top_ind[10:]
                #         # print(top_ind.shape)
    ...
    if val_acc > few_shot_meta_data["best_val_acc"][index]:
        if val_acc > few_shot_meta_data["best_val_acc_ever"][index]:
            ...
            if args.save_features != "":
                ...
                save_feat_dir = os.path.join(str(os.path.dirname(os.path.abspath(__file__))), 'save_features', str(cur_time), 'res50')
                if not os.path.exists(save_feat_dir):
                    os.makedirs(save_feat_dir)
                torch.save(torch.cat([train_features, val_features, novel_features], dim = 0), os.path.join(save_feat_dir, args.save_features + str(args.n_shots[index])))
