...
def ncm(train_features, features, run_classes, run_indices, n_shots, elements_train=None):
    with torch.no_grad():
        ...
        for batch_idx in range(n_runs // batch_few_shot_runs):
            ...
            np.save("./visualization/easy/{}".format(str(batch_idx)), save_visio)
        ...
        if val_acc > few_shot_meta_data["best_val_acc_ever"][index]:
            ...
            if args.save_model != "":
                if len(args.devices) == 1:
                    ...
                    # torch.save(model.state_dict(), args.save_model + str(args.n_shots[index]) + str(args.runs))
                    
                else:
                    torch.save(model.module.state_dict(), args.save_model + str(args.n_shots[index]))
                    # torch.save(model.module.state_dict(), args.save_model + str(args.n_shots[index]) + str(args.runs))
