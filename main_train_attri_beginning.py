...
def train(model, attribute_model, train_loader, optimizer, epoch, scheduler, mixup = False, mm = False):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if mm: # as in method S2M2R, to be used in combination with rotations
            ...
            features_avg = F.avg_pool2d(features, features.shape[2])
            if args.rotations:
                output, _ = output
        ...
        if mixup and args.mm: # mixup or manifold_mixup
            ...
            if args.rotations:
                ...
                loss = ((lam * criterion(output, target) + (1 - lam) * criterion(output, features, target[index_mixup])) + (lam * criterion(output_rot, target_rot) + (1 - lam) * criterion(output_rot, target_rot[index_mixup]))) / 2
            else:
            ...
            if args.rotations:
                ...
                loss += crit(output, features_avg, target)
            ...
            for i in range(matching_list.shape[0]): ###遍历每个配对
                matching_A = features_attri_input[matching_list[i, 0]].permute(1,0)
                matching_B = features_attri_input[matching_list[i, 1]].permute(1,0)
                ...
                # similarity = get_similarity_vectors(matching_A, matching_B)
                similarity = get_similarity_vectors(ori_matching_target_A, ori_matching_target_B)
                post_simi = get_similarity_vectors(matching_A, matching_B)
                ...
                for e in range(pre_simi_min_indx.shape[0]): ###遍历配对中的每个点位
                    ...
                    for ind in range(bottomk_ind.shape[0]): ###遍历每个点位中相似关系，找到配对点位作为match
                        if tag[bottomk_ind[ind]] == 4: ###避免某个相似点被使用次数超过5次
                            continue
                        if item_simi_bottomk[ind] > 10:
                            break
                ...
                # max_simi_ind = similarity.topk(k=4, dim=0, largest=False)[1]
                # max_simi_ind = similarity.max(dim=1)[1]
                # max_post_simi = post_simi.max(dim=1)[1]
                # np.savetxt(os.path.join('./save_tensor',str(i)+'.csv'), matching_B.detach().cpu().numpy(), fmt='%.6f')
                # np.savetxt(os.path.join('./save_tensor',str(i)+'_ori.csv'), ori_matching_target_B.detach().cpu().numpy(), fmt='%.6f')
            loss_contra /= (B//2)
            loss += 0.5*loss_contra

            #######################################
            
            # if args.rotations:
            #     output, output_rot = output
            #     loss = 0.5 * criterion(output, features, target) + 0.5 * criterion(output_rot, features, target_rot)                
            # else:
            #     loss = criterions(output, features, target)
        # backprop loss
...
for i in range(args.runs):
    ...
    if len(args.devices) > 1:
        ...
        attribute_model = torch.nn.DataParallel(model, device_ids = args.devices)
