...
class EpisodicCPUDataset():
    def __init__(self, data, num_classes, transforms = [], episode_size = args.batch_size, use_hd = False):
        ...
        for i in range(episodes):
            ...
            for c in range(args.n_ways):
                ...
                # self.targets += [classes[c]] * (self.episode_size // args.n_ways)
    ...
    def generate_next_episode(self, idx):
        ...
        for c in range(args.n_ways):
            ...
            # self.targets[idx * self.episode_size + c * n_samples: idx * self.episode_size + (c+1) * n_samples] = classes[c] * (self.episode_size//args.n_ways)
    ...
    def __getitem__(self, idx):
        ...
        return self.transforms(elt), self.targets[idx]
        # return self.transforms(elt), self.targets[idx], self.indices
...
def miniImageNet(use_hd = True):
    ...
    for subset in ["train", "validation", "test"]:
        data = []
        target = []
        # with open(args.dataset_path + "mini-imagenet/" + subset + ".csv", "r") as f:
        with open(args.dataset_path + "miniimagenetimages/" + subset + ".csv", "r") as f:
        # with open(args.dataset_path + "new_miniimagenetimages100/" + subset + ".csv", "r") as f:
            start = 0
            for line in f:
                if start == 0:
                    start += 1
                    ...
                    # path = args.dataset_path + "mini-imagenet/" + "images/" + c[:-1] + "/" + fn
                    path = args.dataset_path + "miniimagenetimages/" + "images/" + fn
                    # path = args.dataset_path + "new_miniimagenetimages100/" + "images/" + fn
    ...
    if args.episodic:
        train_loader = episodic_iterator(datasets["train"][0], 64, transforms = all_transforms, forcecpu = True, use_hd = True)
    else:
        train_loader = iterator(datasets["train"][0], datasets["train"][1], transforms = train_transforms, forcecpu = True, use_hd = use_hd)
    train_clean = iterator(datasets["train"][0], datasets["train"][1], transforms = all_transforms, forcecpu = True, shuffle = False, use_hd = use_hd)
    ...
    test_loader = iterator(datasets["test"][0], datasets["test"][1], transforms = all_transforms, forcecpu = True, shuffle = False, use_hd = use_hd)
    # val_loader = episodic_iterator(datasets["validation"][0], 16, transforms = all_transforms, forcecpu = True, use_hd = use_hd)
    # test_loader = episodic_iterator(datasets["test"][0], 20, transforms = all_transforms, forcecpu = True, use_hd = use_hd)
