...
# generated_novel_proto = torch.load('./word_embedding/miniimagenet/0.5attriloss_0.6relationloss_generate_novel_protos_1shot_relation.pkl').to(args.device)
# weighted_novel_proto = torch.load('./word_embedding/miniimagenet/0.5attriloss_0.6relationloss_weighted_novel_protos_1shot_relation.pkl').to(args.device)
# weighted_novel_proto = torch.load('/data1/lzj/easy_mine/word_embedding/miniimagenet/weighted_novel_protos_1shot_absolute_only.pkl').to(args.device)
generated_novel_proto = torch.load('/data1/lzj/easy_mine/word_embedding/cifarfs/generate_novel_protos_1shot_relation.pkl').to(args.device)
weighted_novel_proto = torch.load('/data1/lzj/easy_mine/word_embedding/cifarfs/weighted_novel_protos_1shot_relation.pkl').to(args.device)
# generated_novel_proto = torch.load('/data1/lzj/easy_mine/word_embedding/FC100/0.5attriloss_0.6relationloss_generate_novel_protos_1shot_relation.pkl').to(args.device)
# weighted_novel_proto = torch.load('/data1/lzj/easy_mine/word_embedding/FC100/0.5attriloss_0.6relationloss_weighted_novel_protos_1shot_relation.pkl').to(args.device)
...
def ncm(train_features, features, run_classes, run_indices, n_shots, elements_train=None):
    with torch.no_grad():
        ...
        for batch_idx in range(n_runs // batch_few_shot_runs):
            runs, class_batch = generate_runs(features, run_classes, run_indices, batch_idx)
            # print(class_batch)
            generated_protos = generated_novel_proto[class_batch]
            mean_generated_protos = generated_protos.mean(dim=-1)
            generated_protos = preprocess(train_features, generated_protos, elements_train=elements_train)

            weighted_protos = weighted_novel_proto[class_batch]
            mean_weighted_protos = weighted_protos.mean(dim=-1)
            weighted_protos = preprocess(train_features, weighted_protos, elements_train=elements_train)
            
            means = torch.mean(runs[:,:,:n_shots], dim = 2)
            
            all_sample_num = n_shots+1
            weight_shots = n_shots / all_sample_num
            weight_weighted = 1 / all_sample_num
            
            # 用generate的方法
            # means = weight_shots*means + weight_weighted*generated_protos

            ### 用relation-weight的方法
            nonzero_ind = (mean_weighted_protos != 0.0)
            means[nonzero_ind] = weight_shots*means[nonzero_ind] + weight_weighted*weighted_protos[nonzero_ind]

            ### 用generate和weight的方法
            # all_sample_num = n_shots+2
            # weight_weighted = 1 / all_sample_num
            # weight_non_zero_shots = n_shots / all_sample_num
            # weight_zero_shots = n_shots / (all_sample_num-1)
            # weight_non_zero_generate = 1 / all_sample_num
            # weight_zero_generate = 1 / (all_sample_num-1)
            
            # nonzero_ind = (mean_weighted_protos != 0.0)
            # zero_ind = (mean_weighted_protos == 0.0)
            # means[zero_ind] = weight_zero_shots*means[zero_ind] + weight_zero_generate*generated_protos[zero_ind]
            # means[nonzero_ind] = weight_non_zero_shots*means[nonzero_ind] + weight_non_zero_generate*generated_protos[nonzero_ind] + weight_weighted*weighted_protos[nonzero_ind]

            query = runs[:, :, n_shots:].reshape(runs.shape[0], -1, runs.shape[-1]).detach().cpu()
            supp = means.detach().cpu()
            save_visio = torch.cat([supp, query], dim=-2).numpy()
            np.save("./visualization/GM/{}".format(str(batch_idx)), save_visio)
