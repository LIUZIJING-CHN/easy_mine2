...
### main train function
def train(model, train_loader, optimizer, epoch, scheduler, mixup = False, mm = False, minus_channel=False):
    model.train()
    global last_update
    losses, total = 0., 0
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if mixup and args.mm: # mixup or manifold_mixup
            ...
            if minus_channel:
                output, features = model(data, minus_channel=True)
            else:
        ...
        if (batch_idx + 1 == length) or (time.time() - last_update > 0.1) and not args.quiet:
            if batch_idx + 1 < length:
                print("\r{:4d} {:4d} / {:4d} loss: {:.5f} time: {:s} lr: {:.5f} minus_channel: {:s}".format(epoch, 1 + batch_idx, length, losses / total, format_time(time.time() - start_time), float(scheduler.get_last_lr()[0]), str(minus_channel)), end = "")
            else:
...
def test(model, test_loader):
    ...
    with torch.no_grad():
        ...
        for data, target in test_loader:
            data, target = data.to(args.device), target.to(args.device)
            output, _ = model(data, minus_channel=True)
            if args.rotations:
                output, _ = output
...
def train_complete(model, loaders, mixup = False):
    ...
    for epoch in range(args.epochs + args.manifold_mixup):
        ...
        epoch_tag = (args.epochs + args.manifold_mixup)/2
        tag = epoch >= epoch_tag
        train_stats = train(model, train_loader, optimizer, (epoch + 1), scheduler, mixup = mixup, mm = epoch >= args.epochs, minus_channel=tag)        
        ...
        if (epoch + 1) > args.skip_epochs:
            if few_shot:
                if args.ema > 0:
                    ema.store()
                    ema.copy_to()
                res = few_shot_eval.my_update_few_shot_meta_data(model, train_clean, novel_loader, val_loader, few_shot_meta_data)
                if args.ema > 0:
                    ema.restore()
