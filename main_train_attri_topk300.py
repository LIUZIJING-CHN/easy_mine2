...
import my_few_shot_eval, my_few_shot_eval_kmeans
import resnet
import wideresnet
import resnet12
import attribute_generator_topk300
import s2m2
import mlp
print("models.")
if args.ema > 0:
        ...
        if mixup and args.mm: # mixup or manifold_mixup
            ...
            for i in range(matching_list.shape[0]): ###遍历每个配对
                ...
                for e in range(pre_simi_min_indx.shape[0]): ###遍历配对中的每个点位
                    ...
                    if similarity[pre_simi_min_indx[e]].min() > 12:
                        break
...
def train_complete(model, attribute_model, loaders, mixup = False):
    ...
    for epoch in range(args.epochs + args.manifold_mixup):
        ...
        if args.save_attri_model != "":
            if len(args.devices) == 1:
                torch.save(attribute_model.state_dict(), args.save_attri_model)
            else:
        ...
        if (epoch + 1) > args.skip_epochs:
            if few_shot:
                if args.ema > 0:
                    ema.store()
                    ema.copy_to()
                res = my_few_shot_eval.update_few_shot_meta_data(model, attribute_model, train_clean, novel_loader, val_loader, few_shot_meta_data)
                if args.ema > 0:
                    ema.restore()
...
if few_shot:
    ...
    val_runs = list(zip(*[my_few_shot_eval.define_runs(args.n_ways, s, args.n_queries, val_classes, elements_val) for s in args.n_shots]))
    val_run_classes, val_run_indices = val_runs[0], val_runs[1]
    novel_runs = list(zip(*[my_few_shot_eval.define_runs(args.n_ways, s, args.n_queries, novel_classes, elements_novel) for s in args.n_shots]))
...
def create_attribure():
    return attribute_generator_topk300.Attribute_generator(num_classes=num_classes).to(args.device)
...
if args.test_features != "":
    ...
    if not args.transductive:
        for i in range(len(args.n_shots)):
            val_acc, val_conf, test_acc, test_conf = my_few_shot_eval.evaluate_shot(i, train_features, val_features, test_features, few_shot_meta_data)
            print("Inductive {:d}-shot: {:.2f}% (± {:.2f}%)".format(args.n_shots[i], 100 * test_acc, 100 * test_conf))
    else:
        for i in range(len(args.n_shots)):
            val_acc, val_conf, test_acc, test_conf = my_few_shot_eval.evaluate_shot(i, train_features, val_features, test_features, few_shot_meta_data, transductive = True)
