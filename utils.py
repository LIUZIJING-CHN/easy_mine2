...
def criterion_episodic(features, targets, n_shots = args.n_shots[0]):
    ...
    ##############################################
    # feature = feat[:, n_shots:]
    # feature = feature.reshape(-1, feature.shape[2])
    # f = F.normalize(feature, p=2, dim=feature.dim()-1, eps=1e-12)
    # w = F.normalize(means, p=2, dim=means.dim()-1, eps=1e-12)
    # cls_scores = f @ w.transpose(0, 1) # B, M, nC
    # # cls_scores = self.scale_cls * (cls_scores + self.bias)
    # return torch.nn.CrossEntropyLoss()(cls_scores, targets.reshape(args.n_ways,-1)[:,n_shots:].reshape(-1))
    ##############################################
