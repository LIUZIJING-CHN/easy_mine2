...
import my_few_shot_eval_attri_stage
import resnet
import wideresnet
import resnet12
        ...
        if (epoch + 1) > args.skip_epochs:
            if few_shot:
                if args.ema > 0:
                    ema.store()
                    ema.copy_to()
                res = my_few_shot_eval_attri_stage.update_few_shot_meta_data(model, attribute_model, train_clean, novel_loader, val_loader, few_shot_meta_data)
                if args.ema > 0:
                    ema.restore()
...
if few_shot:
    ...
    val_runs = list(zip(*[my_few_shot_eval_attri_stage.define_runs(args.n_ways, s, args.n_queries, val_classes, elements_val) for s in args.n_shots]))
    val_run_classes, val_run_indices = val_runs[0], val_runs[1]
    novel_runs = list(zip(*[my_few_shot_eval_attri_stage.define_runs(args.n_ways, s, args.n_queries, novel_classes, elements_novel) for s in args.n_shots]))
...
if args.test_features != "":
    ...
    if not args.transductive:
        for i in range(len(args.n_shots)):
            val_acc, val_conf, test_acc, test_conf = my_few_shot_eval_attri_stage.evaluate_shot(i, train_features, val_features, test_features, few_shot_meta_data)
            print("Inductive {:d}-shot: {:.2f}% (± {:.2f}%)".format(args.n_shots[i], 100 * test_acc, 100 * test_conf))
    else:
        for i in range(len(args.n_shots)):
            val_acc, val_conf, test_acc, test_conf = my_few_shot_eval_attri_stage.evaluate_shot(i, train_features, val_features, test_features, few_shot_meta_data, transductive = True)
