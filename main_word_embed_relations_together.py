...
import resnet
import wideresnet
import resnet12_generate_protos as resnet12
import s2m2
import mlp
...
attri_bank = torch.load('/data/lzj/easy_mine/word_embedding/miniimagenet/train_vocab_vec.pkl')
class_relations = torch.load('/data/lzj/easy_mine/word_embedding/miniimagenet/train_vec_simi.pkl')
...
def train(model, train_loader, optimizer, epoch, scheduler, mixup = False, mm = False):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        attri_class_target = attri_bank[class_batch].to(args.device)
        ...
        if mixup and args.mm: # mixup or manifold_mixup
            ...
            output, generate_protos, features = model(data, attri_class_target)
            ...
            ##
            loss_generation = criterion_generation(class_protos, generate_protos)
            ## 直接将词嵌入进行反映射学习
            ...
            if args.rotations:
                output, output_rot = output
                loss = 0.5 * crit(output, features, target) + 0.5 * crit(output_rot, features, target_rot) + 0.5 * loss_generation
                ...
                loss = 0.5 * crit(output, features, target) + 0.5 * loss_generation
